...
EventYoshi.prototype.on = function(event, listener) {
  ...
  if (wrapper === undefined) {
    ...
    var self = this;
    wrapper = this.__wrappers[event] = function() {
...
EventYoshi.prototype.removeListener = function(event, listener) {
  ...
  if (wrapper !== undefined) {
    ...
    var prevLen = ev.length;
    this._removeListener(event, listener);

    // if length is the same, no listener was removed
    var newLen = ev.length;
    if (prevLen === newLen) return;

    // if found, check if wrapper needs to be removed
    if (newLen !== 0) return;
    this.children.forEach(function(ee) {
